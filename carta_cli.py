...
def start_review_session(decks, reviews):
        ...
        if review_type == 0:
                ...
                while True:
                        ...
                        if review_session.check_answer(chosen_option):
...
def command_loop():
        try:
                carta_data = carta_local.load_from_file("carta_data.p")
        except:
                carta_data = {
                        "data_sets": [],
                        "decks": [],
                        "reviews": [] }
        data_sets = carta_data["data_set"]
        ...
        if input("Save data (Y/N)? ").lower() == "y":
                carta_local.save_data(data_sets, decks, reviews, "carta_data.p")
